...
contract WorldFinance is Ownable, ReentrancyGuard {
    ...
    constructor(address _wld, address _rwaNFT) {
        WLD = IERC20(_wld);
        rwaNFT = IRWANFT(_rwaNFT);
        totalLiquidity = 20 ether;
    }
    ...
    function updateRWANFT(address _new) external onlyOwner {
        rwaNFT = IRWANFT(_new);
    }
